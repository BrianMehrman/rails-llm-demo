...
### 4. Missing `/up` health route — NOT FIXED (current blocker)
...
- `3cd2637` fix(deploy): the three fixes above (#1, #2, #3). **WIP — not yet PR'd.**
- (this handoff doc)
...
2. Redeploy: `skaffold run` (rebuilds image — `routes.rb` is baked in). Confirm `kubectl get pods` shows `rails-app 1/1 Running`.
3. Then verify the actual demo path (still untested):
...
4. Commit bug #4 fix, then open a PR (PR-only rule — never merge to main locally).
